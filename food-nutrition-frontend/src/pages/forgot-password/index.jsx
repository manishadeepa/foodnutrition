...
const ForgotPassword = () => {
  return (
    <>
      <Helmet>
        <title>Forgot Password - FoodNutritionAI</title>
        <meta name="description" content="Reset your FoodNutritionAI account password" />
      ...
      <div style={{ position: 'relative', minHeight: '100vh', width: '100%', overflow: 'hidden', background: 'linear-gradient(135deg, #0a1628 0%, #0d1f2d 40%, #0a1a1a 100%)', fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ position: 'relative', zIndex: 10, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '32px 16px', paddingTop: `${NAVBAR_HEIGHT + 32}px` }}>
          ...
          >
            <div style={{ backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', borderRadius: '1.5rem', padding: 'clamp(24px, 5vw, 48px)', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', boxShadow: '0 8px 32px rgba(0,0,0,0.4)', fontFamily: "'DM Sans', sans-serif" }}>
              <WelcomeHeader />
              <ForgotPasswordForm />
              <SecurityBadges />
            </div>
            <div className="text-center mt-6 md:mt-8">
              <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: '13px', fontWeight: 500, fontFamily: "'DM Sans', sans-serif" }}>
                &copy; {new Date()?.getFullYear()} FoodNutritionAI. All rights reserved.
              </p>
            </div>
          </motion.div>
        </div>
